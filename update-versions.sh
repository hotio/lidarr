#!/bin/bash
branch=$(curl -u "${GITHUB_ACTOR}:${GITHUB_TOKEN}" -fsSL "https://api.github.com/repos/lidarr/lidarr/pulls?state=open&base=develop&sort=updated&direction=desc" | jq -re '[sort_by(.updated_at) | .[] | select((.head.repo.full_name == "Lidarr/Lidarr") and (.head.ref | contains("dependabot") | not)) | .head.ref][0]') || exit 1
