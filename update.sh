...
if [[ ${1} == "screenshot" ]]; then
    SERVICE_IP="http://$(ping -c1 -4 service | sed -nE 's/^PING[^(]+\(([^)]+)\).*/\1/p'):8686/system/status"
    NETWORK_IDLE="2"
...
EOF
    ...
    version=$(curl -fsSL "https://services.lidarr.audio/v1/update/develop/changes?os=linux" | jq -r .[0].version)
    [[ -z ${version} ]] && exit 1
